...
func newChannelProvider(conn *ConnectionWrapper, poolSize int, confirmDelivery bool,
	logger LoggerAdapter) (channelProvider, error) {
	if poolSize == 0 {
...
func newPooledChannel(conn *ConnectionWrapper, logger LoggerAdapter, confirmDelivery bool) (*pooledChannel, error) {
	c := &pooledChannel{
		logger,
		conn,
		nil,
...
func (c *pooledChannel) validate() error {
	select {
	case e := <-c.closedChan:
		c.logger.Info("AMQP channel was closed. Opening new channel.", e.Error())
...
func newPooledChannelProvider(conn *ConnectionWrapper, poolSize int, confirmDelivery bool,
	logger LoggerAdapter) (channelProvider, error) {
	logger.Info("Creating pooled channel provider", "pool-size", poolSize)
...
func (p *pooledChannelProvider) Close() {
	...
	p.logger.Info("Closing all channels in the pool", "pool-size", p.channels)

	for _, c := range p.channels {
		if err := c.Close(); err != nil {
			p.logger.Error("Error closing channel: %s", err)
		}
	}
}
