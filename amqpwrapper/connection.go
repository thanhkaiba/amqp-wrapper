...
func (c *ConnectionWrapper) Close() error {
	...
	c.logger.Info("Closing AMQP", nil)
	defer c.logger.Info("Closed AMQP", nil)
...
func (c *ConnectionWrapper) handleConnectionClose() {
	for {
		c.logger.Debug("handleConnectionClose is waiting for amqpwrapper connected", nil)
		<-c.connected
		c.logger.Debug("amqp connected", nil)
...
func (c *ConnectionWrapper) reconnect() {
	...
	if err := backoff.Retry(func() error {
		...
		// should only exit, if closing AMQP
